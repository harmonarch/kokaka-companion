...
function publicUser(row: { id: string; email: string; nickname: string | null }) {
  return {
    id: row.id,
    email: row.email,
...
async function saveRefreshToken(
  ...
  await env.DB.prepare(
    ...
    .bind(crypto.randomUUID(), userId, record.tokenHash, record.expiresAt, Math.floor(Date.now() / 1000))
    .run();
}
...
authRoutes.post("/login", async (c) => {
  ...
  if (!row || !(await verifyPassword(input.password, row.password_hash, c.env))) {
