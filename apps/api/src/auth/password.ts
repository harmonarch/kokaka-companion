...
async function digest(input: string) {
  return toBase64Url(await crypto.subtle.digest("SHA-256", encoder.encode(input)));
}
...
export async function verifyPassword(password: string, stored: string, env: Env) {
  const [salt, hash] = stored.split(".");
  if (!salt || !hash) return false;
  const candidate = await digest(`${salt}:${password}:${env.PASSWORD_HASH_SECRET}`);
