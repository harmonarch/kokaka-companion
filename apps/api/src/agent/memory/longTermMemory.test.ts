...
describe("long-term memory test cases", () => {
  ...
  it.each(memoryTestCases)("has the required fixture shape for $caseId", (testCase) => {
      ...
      const statement = {
        ...
        async first<T>() {
          ...
          if (sql.includes("FROM conversation_summaries")) {
            ...
            const conversationId = this.values[1] ? String(this.values[1]) : null;
        ...
        async run() {
          if (sql.includes("INSERT INTO user_profiles")) {
            const field = sql.match(/INSERT INTO user_profiles \(user_id, (\w+), updated_at\)/)?.[1] as keyof ProfileRow;
            const userId = String(this.values[0]);
...
describe("long-term memory behavior", () => {
  ...
  it("overwrites profile facts and appends event memories", async () => {
    ...
    expect(memories.some((memory) => memory.content.includes("云杉"))).toBe(true);
  });
